...
As of the release `post12`, the wheels are built with MSVC v143 (the default version in VS 2022). However, I just learned that PyTorch is built with MSVC v142 (the default version in VS 2019), and a binary built with a newer MSVC may not work with an older vcredist on the user's computer (see https://learn.microsoft.com/en-us/cpp/porting/binary-compat-2015-2017?view=msvc-170#restrictions , which is a cause of `ImportError: DLL load failed while importing libtriton`). In future it's better to be a bit more conservative and use MSVC v142. 
