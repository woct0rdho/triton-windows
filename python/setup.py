...
class CMakeBuild(build_ext):
    ...
    def build_extension(self, ext):
        ...
        if check_env_flag("TRITON_BUILD_BINARY", "ON"):  # Default ON
            pass
        else:
            cmake_args += ["-DTRITON_BUILD_BINARY=OFF"]
...
if check_env_flag("TRITON_BUILD_PROTON", "ON"):  # Default ON
    ...
    download_and_copy(
        ...
        src_func=lambda system, arch, version: f"cuda_cupti-{system}-{arch}-{version}-archive/lib/cupti64_2024.1.0.dll",
        dst_path="third_party/nvidia/backend/lib/cupti/cupti64_2024.1.0.dll",
