...
Triton 3.2 does not support Blackwell. The main branch of Triton has basic support for Blackwell and some optimizations are still ongoing. However, the main branch of Triton does not work with PyTorch 2.6 and older. You need to compile the `main-windows` branch of this repo (see https://github.com/woct0rdho/triton-windows#build-from-source ), and install the nightly version of PyTorch with CUDA 12.8 .
</details>
...
This is mostly supported by Triton, but fp8 (also known as float8) will not work, see the [known issue](https://github.com/woct0rdho/triton-windows#fp8-is-not-supported-on-rtx-30xx-and-older-gpus).
</details>

<details>
<summary>RTX 20xx (Turing) or older</summary>

This is not officially supported by Triton. It can run some simple AI models, but not always. fp8 (also known as float8) and bf16 (also known as bfloat16) will not work, and you may try changing them to fp16.
</details>
...
Make sure what environment you're using. You can use `Get-Command -All python` in PowerShell (or `where python` in cmd) to see the installation path of Python, and `python --version` to see its version. If you see multiple Python installations, make sure that you install and run everything from the first one.
...
2. Double check your Python version: You can use `Get-Command -All python` in PowerShell (or `where python` in cmd) to see the installation path of Python, and `python --version` to see its version. If you see multiple Python installations, make sure that you install and run everything from the first one
3. If you're using ComfyUI, make sure that you copy-pasted the folders `include` and `libs` from the correct version of Python 
