...
```
It will automatically add the paths containing `cl.exe` and other relevant VS components, see https://github.com/woct0rdho/triton-windows/issues/79
</details>
...
After finding the DLLs, you can install the latest [vcredist](https://aka.ms/vs/17/release/vc_redist.x64.exe), then copy the DLLs `msvcp140.dll`, `vcruntime140.dll`, `vcruntime140_1.dll` from `C:\Windows\System32\` to the Python installation folder, and replace the existing ones.
