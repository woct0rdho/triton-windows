...
This is officially supported by Triton, but fp8 (also known as float8) will not work. I recommend to use GGUF instead of fp8 models in this case.

If you want to run `torch.compile` with fp8, you can try https://github.com/woct0rdho/triton-windows/pull/140 and share your experience.
</details>

<details>
<summary>GTX 16xx/RTX 20xx (Turing architecture, sm75)</summary>

This is not officially supported. It can run some simple AI models, but not always. Things started to break on Turing since Triton 3.3, so I recommend to use Triton 3.2 .
...
```
then it's because currently in Triton, fp8 only works on Nvidia GPUs with sm >= 89, such as RTX 40xx and newer. You may disable fp8 in the node or the code.
